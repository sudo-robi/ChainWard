 ...
 * @dev Accepts health signals from off-chain agents &triggers incidents based on thresholds.
...
contract HealthReporter {
    ...
    function submitHealthSignal(
        ...
        if (!bridgeHealthy) {
            _tryRaiseIncident(
                chainId,
                IncidentManager.FailureType.BridgeStall,
                IncidentManager.IncidentSeverity.Critical,
                IncidentManager.Priority.P1,
                prevBlockNumber,
                prevBlockTimestamp,
    ...
    function _tryAutoResolve(uint256 chainId) internal {
        if (activeIncidentId[chainId] != 0) {
            uint256 incidentId = activeIncidentId[chainId];
            incidents.resolveIncident(incidentId, "Auto-resolved: Health recovery detected");
            activeIncidentId[chainId] = 0;
            emit IncidentAutoResolved(chainId, incidentId);
        }
    }
    ...
    function _tryRaiseIncident(
        ...
        IncidentManager.Priority priority,
        ...
        if (lastIncidentTime[chainId] == 0 || (block.timestamp - lastIncidentTime[chainId]) >= INCIDENT_COOLDOWN) {
            uint256 parentId = activeIncidentId[chainId];
            _raiseNewIncident(chainId, failureType, severity, priority, lastHealthyBlock, lastHealthyTimestamp, description, parentId);
    ...
    function _raiseNewIncident(
        ...
        uint256 incidentId = incidents.raiseIncident(
            chainId,
            failureType,
            severity,
            priority,
